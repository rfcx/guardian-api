...
describe('PATCH /incidents/{id}', () => {
  ...
  test('sets hasOpenIncident to false when a single opened incident is closed', async () => {
    const stream = await Stream.create({ id: 'stream000010', projectId: 'project000010' })
    const incident = await Incident.create({ streamId: stream.id, projectId: stream.projectId, ref: 1 })
    await request(app).patch(`/${incident.id}`).send({ closed: true })
    const s = await Stream.findOne({ where: { id: stream.id } })
    expect(s?.hasOpenIncident).toBe(false)
    await incident.destroy()
    await stream.destroy()
  })
  test('sets hasOpenIncident to false when last opened incident is closed', async () => {
    const stream = await Stream.create({ id: 'stream000011', projectId: 'project000010' })
    const incident1 = await Incident.create({ streamId: stream.id, projectId: stream.projectId, ref: 1 })
    const incident2 = await Incident.create({ streamId: stream.id, projectId: stream.projectId, ref: 2 })
    await request(app).patch(`/${incident1.id}`).send({ closed: true })
    await request(app).patch(`/${incident2.id}`).send({ closed: true })
    const s = await Stream.findOne({ where: { id: stream.id } })
    expect(s?.hasOpenIncident).toBe(false)
    await incident1.destroy()
    await incident2.destroy()
    await stream.destroy()
  })
  test('sets hasOpenIncident to true when an incident is opened', async () => {
    const stream = await Stream.create({ id: 'stream000012', projectId: 'project000010' })
    const incident = await Incident.create({ streamId: stream.id, projectId: stream.projectId, ref: 1, closedAt: '2021-09-16T09:22:00.125Z', closedById: 1 })
    await request(app).patch(`/${incident.id}`).send({ closed: false })
    const s = await Stream.findOne({ where: { id: stream.id } })
    expect(s?.hasOpenIncident).toBe(true)
    await incident.destroy()
    await stream.destroy()
  })
})
