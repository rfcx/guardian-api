...
beforeEach(async () => {
  await truncateDbModels([ClassificationModel, EventModel])
})
afterAll(async () => {
  await stopDb()
})
...
describe('createEvent function', () => {
  test('creates event', async () => {
    ...
    const events: Array<DocumentType<Event>> = await EventModel.find()
    const event = events[0]
    const classifications: Array<DocumentType<Classification>> = await ClassificationModel.find()
